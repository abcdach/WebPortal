...
//####################################################################################		
		...
		switch(isCMD) {
			...
			case "[container]":			p_NUM = 1; break; // NEW
			case "[section]":			p_NUM = 1; break; // NEW
			case "[article]":			p_NUM = 3; break; // NEW
			case "[class]":		 		p_NUM = 1; break; // NEW
			case "codemirror_dg": 		p_NUM = 1; break; // NEW
		...
		if(isCMD === "}"){
			if(Mark_Pointer > 0){
				...
				switch(Current_Mark){
					...
					case "container": isCMD = "<-[container]"; break; // NEW
					case "section":   isCMD = "<-[section]";   break; // NEW
					case "article":   isCMD = "<-[article]";   break; // NEW
					case "class":     isCMD = "<-[class]";     break; // NEW
					default: break;
				}
			}			
		}
...
//####################################################################################
		switch(isCMD) {
			...
			case "<-[w]":
				...
				if(isHTML2.length>0)
				{
					HTML_APPEND(isPage,isHTML2);
				}			
				if(isHTML_CONTENT.length>0)
				{
					HTML_APPEND(isPage,isHTML_CONTENT);
				}
				if(isJAVA.length>0)
				{
					//JAVA_APPEND(isPage,isJAVA);
					$('[data-role="IS_JAVA_SCRIPT"]').append(isJAVA);
				}
				isHTML  = '';
				isHTML2 = '';
				isJAVA  = ''; 
    ...
    //<aside class="bd-sidebar">
      //<nav class="collapse bd-links" id="bd-docs-nav" aria-label="Docs navigation"><ul class="list-unstyled mb-0 py-3 pt-md-1">
				...
				//isHTML2 += '<h2 class="h6 pt-4 pb-3 mb-4 border-bottom">On this page</h2>';
				//isHTML2 += '<nav class="small" id="toc">';
				
			case "[container]":
				Current_Mark = 'container';
				isHTML += '<div class="bd-cheatsheet container-fluid bg-body">';
								
				isHTML2 += '<aside class="bd-aside sticky-xl-top text-muted align-self-start mb-3 mb-xl-5 px-2">';
				isHTML2 += '<h2 class="h6 pt-4 pb-3 mb-4 border-bottom">On this page</h2>';
				isHTML2 += '<nav class="small" id="toc">';
				isHTML2 += '<ul class="list-unstyled">';				
				break;
			case "<-[container]":
				isHTML += '</div>';
				...
				break;
			case "[section]":
				if(isPAYLOAD === "")isPAYLOAD="Content";
				Current_Mark = 'section';				
				isHTML += '<section id="'+isPAYLOAD+'">';
				isHTML += '<h2 class="sticky-xl-top fw-bold pt-3 pt-xl-5 pb-2 pb-xl-3">'+isPAYLOAD+'</h2>';
				isHTML2 += '<li class="my-2">';
				isHTML2 += '<button class="btn d-inline-flex align-items-center collapsed" data-bs-toggle="collapse" aria-expanded="false" data-bs-target="#'+isID+'" aria-controls="'+isID+'">'+isPAYLOAD+'</button>';
				isHTML2 += '<ul class="list-unstyled ps-3 collapse" id="'+isID+'">';
			...
			case "<-[section]":
				...
				break;
			case "[article]":
				if(isPARA[1] === "")isPARA[1]="Article";
				Current_Mark = 'article';
				isHTML += '<article class="my-3" id="'+isPARA[1]+'">';
				isHTML += '<div class="bd-heading sticky-xl-top align-self-start mt-5 mb-3 mt-xl-0 mb-xl-2">';
				isHTML += '	<h3>'+isPARA[1]+'</h3>';	
				if(isPARA[2] !== ""){
					if(isPAYLOAD === "")isPAYLOAD="";
					isHTML += '<a class="d-flex align-items-center" href="'+isPAYLOAD+'">'+isPARA[2]+'</a>';
				}
				isHTML += '</div><div>';	
				isHTML2 += '<li><a class="d-inline-flex align-items-center rounded" href="#'+isPARA[1]+'">'+isPARA[1]+'</a></li>';		
				break;
			case "<-[article]":
				isHTML += '</div></article>';
				break;
			...
			case "codemirror_dg":
				...
				isHTML +='<script>';
				isHTML +='var editor = CodeMirror(document.getElementById("'+isID+'"), ';
				isHTML +='{ lineNumbers: true,tabSize: 4,indentUnit: 4,indentWithTabs: true, mode: "simplemode"}); ';
				isHTML +='editor.setOption("theme", "duotone-light");';
				isHTML +='editor.setValue(`'+isPAYLOAD+'\n`);';
				isHTML +='</script>';
				break;	
